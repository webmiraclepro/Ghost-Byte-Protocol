...
const Contact = ({ actionText, email, phone, discord, mainActionText, extraActionText }) => {
  return (
    <div id="contact" className="bg-bgsecondary">
      <div className="items-center justify-center flex flex-col lg:pb-24 lg:pt-24  lg:flex-row lg:space-x-12 bg-bgsecondary 2xl:container 2xl:mx-auto">
        ...
        <div className="w-full lg:w-2/6 flex flex-col justify-center items-center lg:items-start space-y-8 lg:pb-24 lg:pt-24">
          ...
          <div style={{ width: '473px', height: '70px' }}>
            <input type='email' id="sender" className=" ml-24 lg:ml-0 h-4/5 w-3/5 lg:w-5/6 lg:h-full xl:w-full bg-bgblock shadow-block text-white opacitey-50 p-4 rounded-large justify-center" placeholder="Email"></input>
          </div>
          <div style={{ width: '473px', height: '70px' }}>
            <input type='text' id="subject" className="ml-24 lg:ml-0  h-4/5 w-3/5 lg:w-5/6 lg:h-full xl:w-full bg-bgblock shadow-block text-white opacitey-50 p-4 rounded-large" placeholder="Subject"></input>
          </div>
          <div style={{ width: '473px', height: '200px' }}>
            <textarea type='text' id="content" cols='40' rows='5' className="ml-24 lg:ml-0 h-4/5 w-3/5 lg:w-5/6 lg:h-full xl:w-full bg-bgblock shadow-block text-white opacitey-50 p-4 rounded-large" placeholder="Content..."></textarea >
          </div>
          <div style={{ width: '473px', height: '70px' }}>
            <button
              onClick={() => ContactUs()}
              style={{ background: 'linear-gradient(151.46deg, #01F2FF -14.78%, #0142FF 113.99%)' }}
              className="ml-24 lg:ml-0 h-4/5 w-3/5 capitalize font-body font-medium text-contactDes text-center text-white transition-all rounded-large focus:ring2 lg:shadow-2xl lg:ring-2 lg:w-5/6 lg:h-full xl:w-full"
            >
              {extraActionText}
            </button>
